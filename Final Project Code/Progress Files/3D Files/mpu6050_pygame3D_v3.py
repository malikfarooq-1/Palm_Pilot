...
# 3D Free Roam with Arrow vector v3
# Added dual screen implementation (Render monitor and cockpit), arrow view also created.
# Removed single screen setup
# November 25, 2025
...
pygame.display.set_caption("Drone Flight V4")
...
def render_free_roam(surface, d_roll, d_pitch, d_yaw, cam_x, cam_z):
    ...
    # Add shadow to make it more polished
    shadow_y = cy + 60
    pygame.draw.ellipse(surface, (10, 10, 10), (cx-40, shadow_y, 80, 20))
    ...
    # HUD Drawing
    v_speed = math.hypot(vel_x, vel_z) * 10 
...
def render_cockpit(surface, d_roll, d_pitch, d_yaw, vx, vz):
    surface.fill((0, 0, 0))
    ...
    # Render Text
    txt_surf = big_font.render(f"MOVING: {status_text}", True, (255, 255, 255))
    txt_rect = txt_surf.get_rect(center=(cx, TFT_H - 30))
    surface.blit(txt_surf, txt_rect)
    
    # 4. Angle Stats (Top corners)
    surface.blit(arrow_font.render(f"P: {d_pitch:.1f}", True, (150, 150, 150)), (10, 10))
    surface.blit(arrow_font.render(f"R: {d_roll:.1f}", True, (150, 150, 150)), (TFT_W - 90, 10))
...
try:
    while True:
        ...
        # Basic Physics
        vel_z += (dz_pitch * 0.2) * SPEED_SCALAR
        
        vel_x += (dz_roll * 0.2) * SPEED_SCALAR
        ...
        d_cam_x = vel_z * math.sin(rad_yaw) + vel_x * math.cos(rad_yaw)
        d_cam_z = vel_z * math.cos(rad_yaw) - vel_x * math.sin(rad_yaw)
        
        cam_x += d_cam_x
        cam_z += d_cam_z

        # Render both modes
        render_free_roam(screen_hdmi, roll, pitch, yaw, cam_x, cam_z)
        render_cockpit(screen_tft, roll, pitch, yaw, vel_x, vel_z)
